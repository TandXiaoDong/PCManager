...
namespace ShikuIM.Model
{
    ...
    public abstract class DbHelperSQLite
    {
        ...
        #region 公用方法
        public static bool GetTable(string TabName)
        {
            try
            {
                using (SQLiteConnection connection = new SQLiteConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        int count = -1;
                        int.TryParse(ds.Tables[0].Rows[0]["tab"].ToString(), out count);
                        //打印
        ...
        public static int GetMaxID(string FieldName, string TableName)
        {
            ...
            return GetSingle<int>(strsql);
        }
        public static bool Exists(string strSql)
        {
            return GetSingle<int>(strSql) != 0;
        }
        public static bool Exists(string strSql, params SQLiteParameter[] cmdParms)
        {
            return GetSingle<int>(strSql, cmdParms) != 0;
        }
        ...
        public static T GetSingle<T>(string SQLString)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                using (SQLiteCommand cmd = new SQLiteCommand(SQLString, connection))
                {
                    try
                    {
                        ...
                        {
                            if (obj is T res)//If is T type, return result
                            {
                                return res;
                            }
                            else
        ...
        /// <param name="SQLString">计算查询结果语句</param>
        /// <returns>查询结果（object）</returns>
        public static T GetSingle<T>(string SQLString, params SQLiteParameter[] cmdParms)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                using (SQLiteCommand cmd = new SQLiteCommand())
